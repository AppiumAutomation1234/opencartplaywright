...
```ts
  ...
  44  |         try{
  45  |             await this.txtLastName.fill(lastName);
  46  |         }catch(error){
  47  |             console.log(`Error while entering last name: ${error}`);
  48  |             throw error; // Rethrow the error after logging it
  49  |         }
  50  |         await this.txtLastName.fill(lastName);
  51  |     }
  52  | 
  53  |     async enterEmail(email: string): Promise<void>{
  54  |         try{
  55  |             await this.txtEmail.fill(email);
  56  |         }catch(error){
  57  |             console.log(`Error while entering email: ${error}`);
  58  |             throw error; // Rethrow the error after logging it
  59  |         }   
  60  |         await this.txtEmail.fill(email);
  61  |     }
  62  | 
  63  |     async enterTelephone(telephone: string): Promise<void>{
  64  |         try{
  65  |             await this.txtTelephone.fill(telephone);
  66  |         }catch(error){
  67  |             console.log(`Error while entering telephone: ${error}`);
  68  |             throw error; // Rethrow the error after logging it
  69  |         }  
  70  |         await this.txtTelephone.fill(telephone);
  71  |     }
  72  | 
  73  |     async enterPassword(password: string): Promise<void>{
  74  |         try{
  75  |             await this.txtPassword.fill(password);
  76  |         }catch(error){
  77  |             console.log(`Error while entering password: ${error}`);
  78  |             throw error; // Rethrow the error after logging it
  79  |         }   
  80  |         await this.txtPassword.fill(password);
  81  |     }
  82  | 
  83  |     async enterConfirmPassword(confirmPassword: string): Promise<void>{
  84  |         try{
  85  |             await this.txtConfirmPassword.fill(confirmPassword);
  86  |         }catch(error){
  87  |             console.log(`Error while entering confirm password: ${error}`);
  88  |             throw error; // Rethrow the error after logging it
  89  |         }
  90  |         await this.txtConfirmPassword.fill(confirmPassword);
  91  |     }
  92  | 
  93  |     async checkPrivacyPolicy(): Promise<void>{
  94  |         try{
  95  |             await this.chkPrivacyPolicy.check();
  96  |         }catch(error){
  97  |             console.log(`Error while checking privacy policy: ${error}`);
  98  |             throw error; // Rethrow the error after logging it
  99  |         }
  100 |         await this.chkPrivacyPolicy.check();
  101 |     }
  102 | 
  103 |     async clickOnContinueButton(): Promise<void>{
  104 |         try{
  105 |             await this.btnContinue.click();
  106 |         }catch(error){
  107 |             console.log(`Error while clicking on Continue button: ${error}`);
  108 |             throw error; // Rethrow the error after logging it
  109 |         }
> 110 |         await this.btnContinue.click();
      |                                ^ Error: locator.click: Test timeout of 30000ms exceeded.
  111 |     }   
  112 | 
  113 |     async getConfirmationMessage(): Promise<string>{
  114 |         return await this.msgConfirmation.textContent() ??'';
  115 |     }
  116 | 
  117 |         async registerAccount(userData: { 
  118 |             firstName: string; 
  119 |             lastName: string; 
  120 |             email: string; 
  121 |             telephone: string; 
  122 |             password: string; 
  123 |             confirmPassword: string 
  124 |         }): Promise<void>{   
  ...
  130 |         await this.enterConfirmPassword(userData.confirmPassword);
  131 |         await this.checkPrivacyPolicy();
  132 |         await this.clickOnContinueButton();   
  133 | 
  134 | }}
```